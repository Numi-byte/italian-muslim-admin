...
const PrayerTimesPage: React.FC = () => {
  ...
  const [savingType, setSavingType] = useState<"adhan" | "jamaat" | null>(
    null
  );
    ...
    if (error) {
      ...
      for (const row of existing) {
        map.set(row.prayer, {
          ...
          jamaat_time: row.jamaat_time ? row.jamaat_time.slice(0, 5) : "",
        });
      }
  ...
  const handleCopyFromPreviousDay = async () => {
    ...
    for (const row of existing) {
      map.set(row.prayer, {
        ...
        jamaat: row.jamaat_time ? row.jamaat_time.slice(0, 5) : "",
      });
    }
  ...
  const handleSaveColumn = async (column: "start_time" | "jamaat_time") => {
    if (!selectedMasjidId || !selectedDate) return;

    const processType = column === "start_time" ? "adhan" : "jamaat";
    const label = column === "start_time" ? "Adhan" : "Jamā‘ah";

    setSavingType(processType);
    setMessage(null);
    setMessageType(null);
    ...
    try {
      const payload = rows.map((r) => ({
        ...
        date: r.date,
        prayer: r.prayer,
        [column]: r[column] ? `${r[column]}:00` : null,
      }));
      ...
      if (error) {
        console.error(`Error upserting ${column} in masjid_prayer_times`, error);
        setMessage(error.message);
        setMessageType("error");
      } else {
        await loadPrayerTimes(selectedMasjidId, selectedDate);
        setMessage(`${label} times saved for this day.`);
        setMessageType("success");
        setDirty(false);
        setLastSavedAt(
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4 space-y-4">
        ...
        <div className="pt-2 flex justify-end gap-2">
          <button
            type="button"
            onClick={() => void handleSaveColumn("start_time")}
            disabled={savingType !== null || !selectedMasjidId}
            className="px-4 py-2 rounded-lg border border-emerald-400/70 text-emerald-200 text-xs font-semibold hover:bg-emerald-500/10 disabled:opacity-60"
          >
            {savingType === "adhan" ? "Saving Adhan…" : "Save Adhan times"}
          </button>
          <button
            type="button"
            onClick={() => void handleSaveColumn("jamaat_time")}
            disabled={savingType !== null || !selectedMasjidId}
            className="px-4 py-2 rounded-lg bg-emerald-500 text-emerald-950 text-xs font-semibold disabled:opacity-60"
          >
            {savingType === "jamaat" ? "Saving Jamā‘ah…" : "Save Jamā‘ah times"}
          </button>
        </div>
      </div>
    </div>
